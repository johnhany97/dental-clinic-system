...
public class Application {
	 ...
	 *
	 * Checks if the db is setup, if it is, directs user to Login screen Otherwise,
	 * directs user to SetupWizard
	 */
		...
		if (isSetup()) {
			...
			if (args.length != 0 && args[0].equals("dev")) { // Dev state, re-initialize the db everytime
				try {
					...
					JOptionPane.showMessageDialog(window, e.getMessage(), "Error setting up DB",
