...
public class PayView implements Screen {
	...
	public PayView(DisplayFrame frame, Appointment appointment) throws SQLException {
		...
		if (this.usage != null) this.healthplan = appointment.getPatient().getUsage().getHealthPlan();
		initialize();
	}
	...
	private void initialize() throws CommunicationsException, SQLException {
		...
		for (int i = 0; i < selectedTreatments.size(); i++) {
		    ...
		    String toAdd = "<h3><strong>" + selectedTreatments.get(i) + "</strong> | £" + String.valueOf(price) + "</h3><br>"; 
		    text += toAdd;
		}
		text += "<h2><strong>Total price for appointment:</strong> £" + String.valueOf(this.appointment.calculateCost()) + "</h2>";
		if (this.usage != null) { //has healthplan
			...
			if (type.equals("Cleaning")) {
				if (this.usage.getHygieneUsed() < this.healthplan.getHygieneLevel()) { //Discounted from healthplan
					text += "<h2><strong>Health Plan deduction:</strong> -£" + String.valueOf(this.appointment.calculateCost()) + "</h2><br>";
