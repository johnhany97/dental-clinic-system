...
public class EmployeesScreen implements Screen {
	 ...
	 * 
	 * @param frame
	 *            DisplayFrame in which this is to be shown
	 */
				...
				for (int i = 0; i < 3; i++) {
					...
					if (role.equals("Secretary")) {
						...
						try {
							...
							JOptionPane.showMessageDialog(frame, "Error with the database statement execution.",
	...
	/**
	 * Function used to add fields and labels for each employee (3 is the current
	 * number)
	 */
	...
	private void addSetOfTextFieldsAndLabels() {
		...
		for (int i = 1; i < LABELS.length; i++) { // Start from 1 because 0 is title of screen
			...
			this.labels.get(index).setFont(new Font("Sans Serif", Font.PLAIN, DisplayFrame.FONT_SIZE / 2));
