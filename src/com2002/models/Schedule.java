...
public class Schedule {
	...
	public static ArrayList<Appointment> getAppointmentsByDay(Date date) throws CommunicationsException, SQLException {
		...
		try {
			ResultSet apps = DBQueries.execQuery(
					"SELECT * FROM Appointments WHERE (StartDate <= '" + startDay + "' AND EndDate >= '" + startDay
							+ "') " + "OR (StartDate >= '" + startDay + "' AND StartDate <= '" + endDay + "')",
					conn);
			while (apps.next()) {
	 ...
	 * 
	 * @param doctor
	 *            A doctor, either hygienist or dentist.
	 ...
	 *             for any other error, could be incorrect parameters.
	 */
	public static ArrayList<Appointment> getAppointmentsByDoctor(Doctor doctor)
	...
	/**
	 * Returns all the appointments by doctor and day as an ArrayList of
	 * Appointments.
	 * 
	 * @param doctor
	 *            A doctor, either hygienist or dentist.
	 ...
	 *             for any other error, could be incorrect parameters.
	 */
	public static ArrayList<Appointment> getAppointmentsByDoctorAndDay(Doctor doctor, Date date)
			throws CommunicationsException, SQLException {
		ArrayList<Appointment> appointments = new ArrayList<Appointment>();
		Connection conn = Database.getConnection();
		String username = doctor.getUsername();
		...
		String endDay = year + "-" + month + "-" + day + " 23:59:59.0";
		try {
			ResultSet rs = DBQueries.execQuery("SELECT * FROM Appointments WHERE ((StartDate <= '" + startDay
					+ "' AND EndDate >= '" + startDay + "') " + "OR (StartDate >= '" + startDay + "' AND StartDate <= '"
					+ endDay + "')) AND Username = '" + username + "'", conn);
			while (rs.next()) {
				Timestamp startDate = rs.getTimestamp("StartDate");
				appointments.add(new Appointment(startDate, username));
			}
	...
	/**
	 * Returns a list of appointments for the given doctor for patients who have
	 * similar first and last name to the given parameters.
	 * 
	 * @param doctor
	 *            instance of a doctor
	 * @param firstName
	 *            patient's first name, can be partial
	 * @param lastName
	 *            patient's last name, can be partial
	 ...
	 *             for any other error, could be incorrect parameters.
	 */
	public static ArrayList<Appointment> getAppointmentsByDocAndNameAndDate(Doctor doctor, String firstName,
			String lastName, Date date) throws CommunicationsException, SQLException {
		ArrayList<Appointment> appointments = new ArrayList<Appointment>();
		Connection conn = Database.getConnection();
		try {
			ResultSet patients = DBQueries.execQuery("SELECT PatientID FROM Patients WHERE FirstName LIKE '%"
					+ firstName + "%' AND LastName LIKE '%" + lastName + "%'", conn);
			while (patients.next()) {
				...
				String startDay = year + "-" + month + "-" + day + " 00:00:00.0";
				String endDay = year + "-" + month + "-" + day + " 23:59:59.0";
				ResultSet apps = DBQueries
						.execQuery(
								"SELECT * FROM Appointments WHERE PatientID = " + patID + " AND Username = '" + username
										+ "' AND ((StartDate <= '" + startDay + "' AND EndDate >= '" + startDay
										+ "') OR (StartDate >= '" + startDay + "' AND StartDate <= '" + endDay + "'))",
								conn);
				while (apps.next()) {
					Timestamp startDate = apps.getTimestamp("StartDate");
	 ...
	 * 
	 * @param username
	 *            The username of a doctor.
	 * @param patient
	 *            An instance of a particular Patient.
	 ...
	 *             for any other error, could be incorrect parameters.
	 */
	public static ArrayList<Appointment> getDoctorAppointmentsByPatient(String username, Patient patient)
			throws CommunicationsException, SQLException {
		ArrayList<Appointment> appointments = new ArrayList<Appointment>();
		Connection conn = Database.getConnection();
	 ...
	 * 
	 * @param appointment
	 *            An instance of a particular Appointment.
	 * @throws CommunicationsException
	 *             when an error occurs whilst attempting connection.
	 * @throws SQLException
	 *             for any other error, could be incorrect parameters.
