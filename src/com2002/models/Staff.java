...
public abstract class Staff {
	...
	public Staff(String username) throws CommunicationsException, SQLException {
		...
		try {
			ResultSet rs = DBQueries.execQuery("SELECT * FROM Employees WHERE username = '" + username + "'", conn);
			if (rs.next()) {
	...
	public Staff(String username, String password) throws CommunicationsException, SQLException {
		Connection conn = Database.getConnection();
		try {
			ResultSet rs = DBQueries.execQuery(
	...
	/**
	 * This constructor is called whenever a new Doctor or Secretary staff member is
	 * created.
	 * 
	 * @param firstName
	 ...
	 * @param username
	 *            Username of staff member, needs to be unique.
	 * @param password
	...
	public Staff(String firstName, String lastName, String username, String password, Role role)
		...
		if (!DBQueries.staffUsernameExists(username)) {
			DBQueries.execUpdate("INSERT INTO Employees VALUES ('" + firstName + "', '" + lastName + "', '" + username
					+ "', '" + password + "', '" + getRoleString(role) + "')");
			...
			throw new MySQLIntegrityConstraintViolationException(
					"A staff member with username " + username + " already exists.");
		}
	}
	...
	public void setFirstName(String firstName) throws CommunicationsException, SQLException {
		DBQueries
				.execUpdate("UPDATE Employees SET FirstName = '" + firstName + "' WHERE Username = '" + username + "'");
		this.firstName = firstName;
	}
	...
	public void setLastName(String lastName) throws CommunicationsException, SQLException {
		DBQueries.execUpdate("UPDATE Employees SET LastName = '" + lastName + "' WHERE Username = '" + username + "'");
		this.lastName = lastName;
	}
	...
	public void setUsername(String username)
		...
		DBQueries.execUpdate(
				"UPDATE Employees SET Username = '" + username + "' WHERE Username = '" + this.username + "'");
		this.username = username;
	}
	 ...
	 * 
	 * @param role
	 *            The new role you want the staff member to be.
	 * @throws CommunicationsException
	 *             when an error occurs whilst attempting connection
	 * @throws SQLException
	 *             for any other error, could be incorrect parameters.
	...
	public void setRole(Role role) throws CommunicationsException, SQLException {
		DBQueries.execUpdate(
				"UPDATE Employees SET Role = '" + getRoleString(role) + "' WHERE Username = '" + username + "'");
		this.role = getRoleString(role);
	}
	 ...
	 * 
	 * @param role
	 *            Role of staff member as an enum.
